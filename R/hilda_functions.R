...
#'
#'
visPMS <- function(vF, numBases, baseCol = NA, trDir, charSize = 1.2, scale = TRUE, alpha = 2, charLimit = 0.25) {
...
hilda_plotSignature <- function(inputParam) {
    ...
    for (i in 1:inputParam@signatureNum) {
        ...
        polygon(c(-0.25, 6.25, 6.25, -0.25), c(-0.25, -0.25, 3.09, 3.09), col = "white",
                border = "white")
...
hilda_bayesfactor <- function(inputG = G, inputParam = Param, refGroup, sigOrder = NULL,
    ...
    model.fit<-R2jags::jags(model.file = modelFile,
                            data = jdata, parameters.to.save = var.s,
...
hilda_test <- function(inputG = G, inputParam = Param, refGroup, sigOrder = NULL,
    ...
    modelFile <- system.file("model/hilda.txt", package = "HiLDA")
